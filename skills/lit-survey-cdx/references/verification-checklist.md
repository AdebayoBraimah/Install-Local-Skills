...
- Count papers assigned to that section via Keywords search
...
rg -l '\\[\\[Hub-Name\\]\\]' Ideas/Research -g '*.md' | wc -l
```

## Vault Link Integrity

Find broken wikilinks in all new and modified notes:
1. Search for all `[[wikilinks]]` in survey-related notes with `rg`
...
- Search for papers from the last 2-3 years (check Date Created or year in citation)
...
1. Search for `![[*.png]]` and `![[*.drawio]]` patterns in new notes with `rg`
