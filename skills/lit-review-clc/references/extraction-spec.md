...
- **Inkscape** (invoke `/inkscape` for full reference) for programmatic SVG creation with precise geometry, editing extracted vector figures, and figure recreation when extraction fails
- Prefer Mermaid when sequential/branching; Excalidraw when spatial; Inkscape when precise control or vector editing needed
- See `references/diagram-tools.md` (in lit-summarizer) for full decision logic
...
1. Grep for `Zotero-Key: "KEY"` in `Ideas/Research/`
2. Glob for `{citationKey}.md` in `Ideas/Research/`
3. If match found:
   - Empty shell -> upgrade in-place via Edit
