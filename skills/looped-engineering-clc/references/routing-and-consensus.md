...
| Route | Trigger |
| --- | --- |
| `judgement-engineering-clc` | Fuzzy, contradictory, high-impact, under-specified, hidden-assumption driven, or drifting goals. |
...
| `claude-api` | Claude/Anthropic API and SDK behavior, model ids/pricing/limits, prompt caching, tool use, MCP, agents, and Claude Code/Anthropic platform implementation questions. |
...
| `lit-review-clc` | Scoped literature reviews that need search, triage, summarize, synthesize, and verify phases. |
...
| `plan-review-clc` | Substantial plan before execution, Full OS plan, or bounded Standard formal-review trigger. |
...
- Use `claude-api` before general web or research routes for Claude/Anthropic APIs, model selection, pricing/limits, prompt caching, tool use, or Claude Code behavior.
...
- Use `deep-research-academic`, `academic-researcher`, or `lit-review-clc` for academic sources, papers, literature review, or research methodology.
...
In Claude Code, spawn reviewer/planner/specialist roles with the `Agent` tool. Run independent roles concurrently by issuing their `Agent` calls in a single message; use `subagent_type` to pick a specialized agent (for example `Explore` for read-only investigation, `Plan` for architecture planning, or `general-purpose` otherwise) and `mode` to set the permission posture.

Subagents cannot invoke skills with the Skill tool. To give a subagent a skill's behavior, read the relevant `SKILL.md` (and any required reference files) from `~/.claude/skills/<skill>/` in the orchestrator, then embed the needed instructions directly in the `Agent` prompt. The orchestrator itself still invokes skills such as `judgement-engineering-clc`, `plan-review-clc`, and `tdd` through the Skill tool.
...
- Medium conflicts defer to the more specialized reviewer or `plan-review-clc` execution reviewer and require a documented resolution.
...
If subagents are unavailable or not authorized, use inline named review roles with the same statuses, inputs, findings, confidence, and signoff expectations. Spawn actual subagents via the `Agent` tool when available and authorized by the user/request or by a skill such as `plan-review-clc`.
...
- Invocation mechanism: use `judgement-engineering-clc`, or an inline Judgement reviewer role if the skill is unavailable.
...
- Invocation mechanism: use `plan-review-clc`; if it cannot run, use an inline Plan reviewer role and record the limitation.
...
- Expected output artifact: `plan-review-clc` review result or inline plan review result.
