...
description: |
  ...
  across 100-350+ papers. Composes with /lit-summarizer-clc for batch summarization.
  ...
  Invoke as /lit-survey-clc "topic description" or /lit-survey-clc with no args for interactive setup.
...
Subagents cannot invoke skills. Read these SKILL.md files and embed relevant excerpts in agent dispatch prompts:

| Agent Type | Embedded Skills | Read From |
|---|---|---|
| Search Agents | `deep-research-academic` | `~/.claude/skills/deep-research-academic/SKILL.md` |
| Hub Writer Agents | `academic-researcher` + `mermaid-diagrams` | `~/.claude/skills/academic-researcher/SKILL.md`, `~/.claude/skills/mermaid-diagrams/SKILL.md` |
| Synthesis Agents | `academic-researcher` | `~/.claude/skills/academic-researcher/SKILL.md` |
...
- Grep `Ideas/Research/` for notes related to the topic (search Keywords, tags, titles)
- Grep for existing hub notes (`ad-tldr` tag or `hub-note` tag) across `Ideas/` and `Ideas/Research/`
...
- CLI tools exist: `which pdfimages magick cli-anything-zotero cli-anything-gimp`
...
4. Dispatch 4 agents in parallel (A-D), one per database:

| Agent | Database | Tool |
|---|---|---|
| A | Google Scholar | `scholarly` via `scripts/scholarly_search.py` (Bash) |
| B | Semantic Scholar | WebFetch (`api.semanticscholar.org`) |
...
**Delegate to `/lit-summarizer-clc`**.

The orchestrator reads `~/.claude/skills/lit-summarizer-clc/SKILL.md` and replicates its pipeline logic (subagents cannot invoke skills). lit-summarizer handles:
1. **Extractor Agent** (1) — pulls Zotero metadata for the collection, builds paper queue
2. **Summarizer Agents** (5 per wave) — creates structured literature notes with figures, Mermaid, math, tables
...
Completion detection: sentinel comment `<!-- HUB-COMPLETE: {agent-id} -->` at end of last file. Orchestrator checks via Grep.
...
Completion detection: check for all output files via Glob.
...
- Summarizer waves: 5 per wave (via lit-summarizer)
...
Phase 2 — Summarize (via lit-summarizer) → git commit → non-blocking spot-check
