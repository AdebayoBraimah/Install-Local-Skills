...
| B | Semantic Scholar | WebFetch (`api.semanticscholar.org`) | Free API, no auth. Returns DOI, abstract, citation count, venue. |
| C | arXiv | WebSearch + WebFetch (`arxiv.org/search`) | Preprints and recent work. |
| D | IEEE/ACM + DBLP | WebSearch + WebFetch | Peer-reviewed venues. |
...
2. **CrossRef** (fallback for DOI): WebFetch `https://api.crossref.org/works/{doi}` for authoritative venue/volume/pages
3. **Semantic Scholar** (fallback for abstract/citations): WebFetch `https://api.semanticscholar.org/graph/v1/paper/search?query={title}` for abstract, DOI, citation count
...
- Check against existing vault: `cli-anything-zotero search "{title}"` and Grep `Ideas/Research/` for matching citation keys
