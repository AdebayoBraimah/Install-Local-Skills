...
4. **Against existing vault**: Run `cli-anything-zotero search "{title}"` to check Zotero library. Grep `Ideas/Research/` for matching citation keys.
...
2. **CrossRef** (fallback for DOI): WebFetch `https://api.crossref.org/works/{DOI}`

3. **Semantic Scholar** (fallback): WebFetch `https://api.semanticscholar.org/graph/v1/paper/search?query={TITLE}&limit=1&fields=title,authors,year,venue,abstract,citationCount,externalIds`
