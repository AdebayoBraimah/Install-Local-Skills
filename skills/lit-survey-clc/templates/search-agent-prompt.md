...
### If Agent C (arXiv)
Use WebSearch for `site:arxiv.org {QUERY}` and WebFetch for individual paper pages.

### If Agent D (IEEE/ACM + DBLP)
Use WebSearch for `site:ieeexplore.ieee.org {QUERY}` and `site:dl.acm.org {QUERY}`. Also search DBLP: `https://dblp.org/search?q={QUERY}`.
