...
- `finding` — a single finding JSON object (file, line, severity, category, title,
  body, suggestion).
- `diff` — the unified diff under review (`/tmp/gr-diff.patch`).
- `context` — the Context Pack (`/tmp/gr-context.md`).
...
less than a confidently-wrong one in the report. Your final message is the JSON —
output no prose.
