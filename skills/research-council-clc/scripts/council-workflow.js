...
if (ranCode) {
  ...
  cleanup = await agent(
    ...
    `1. Run \`git worktree prune -v\` in the repository to drop stale worktree admin entries, then \`git worktree list\` and report any worktrees that remain. If a workflow-generated repro worktree lingers with leftover files (its path is under \`.claude/worktrees/\` and/or its branch is named \`worktree-wf_*\`), \`git worktree remove --force\` it AND then \`git branch -D\` its dangling \`worktree-wf_*\` branch ref so it does not accumulate — but never remove the main working tree, never delete a user-named branch, and never touch a worktree that holds the user's own uncommitted work.\n` +
