...
Command reference for `~/anaconda3/bin/cli-anything-zotero`.

Global options: `--data-dir PATH` (Zotero data directory), `--version`, `--help`.

Machine-readable item metadata is available through `export -f json`, including collection exports via `export -c "COLLECTION" -f json`. Do not use a global `--json` flag with `items` or `info`; this CLI does not expose one.
...
| `export KEY [KEY...]` | Export items or collections to BibTeX/JSON/CSV | `-f/--format`, `-c/--collection`, `-o/--output` |
...
`items` produces human-readable text. For JSON, use `export -f json`:

```bash
cli-anything-zotero export ABC12345 -f json
cli-anything-zotero export -c "LLM pruning" -f json
```
...
cli-anything-zotero export -c "MARL" -f json -o marl.json
```
