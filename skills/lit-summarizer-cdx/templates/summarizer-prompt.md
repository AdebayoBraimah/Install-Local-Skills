...
- Search with `rg 'Zotero-Key: "{ZOTERO_KEY}"'` in `{VAULT_PATH}/Ideas/Research/`
- Use `find` or `rg --files` to look for `{CITATION_KEY}.md` in `{VAULT_PATH}/Ideas/Research/`

If found:
- **Empty shell** (no summary content): Upgrade in-place. PRESERVE existing frontmatter fields. ADD `Zotero-Key` and `Source-Quality`. Fill content sections.
- **Has user content**: Additive merge only. Append missing sections. Do NOT overwrite existing content.
...
- If PDF exists: read/extract the PDF with available local tools
  - 1-20 pages: read full
  - 20-50 pages: multiple targeted reads with page ranges
...
Skip post-processing if figure already looks good after inspecting it.
